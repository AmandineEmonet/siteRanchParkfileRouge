<?php
    ...
    if(!empty($_POST)){
        ...
        !empty($_POST['name_user']) && !empty($_POST['first_name_user']) && !empty($_POST['mail_user']) && !empty($_POST['password_user']) && !empty($_POST['confpwd']) ) { 
            ...
            $name_user = htmlspecialchars(trim($_POST['name_user'])); // On récupère le nom (trim pour enlever les espaces avant et après)
            $first_name_user = htmlspecialchars(trim($_POST['first_name_user'])); // on récupère le prénom
